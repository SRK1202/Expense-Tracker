...
const Analytics = ({ transactions }) => {
  ...
  const normalized = (item) => (item.transactionType || item.type || "").toString().toLowerCase();
  const toNumber = (v) => parseFloat(v || 0) || 0;

  const totalIncomeTransactions = transactions.filter(
    (item) => normalized(item) === "credit"
  );
  const totalExpenseTransactions = transactions.filter(
    (item) => normalized(item) === "expense"
  );

  let totalIncomePercent = TotalTransactions ? (totalIncomeTransactions.length / TotalTransactions) * 100 : 0;
  let totalExpensePercent = TotalTransactions ? (totalExpenseTransactions.length / TotalTransactions) * 100 : 0;

  const totalTurnOver = transactions.reduce(
    (acc, transaction) => acc + toNumber(transaction.amount),
    0
  );
  const totalTurnOverIncome = transactions
    .filter((item) => normalized(item) === "credit")
    .reduce((acc, transaction) => acc + toNumber(transaction.amount), 0);
  const totalTurnOverExpense = transactions
    .filter((item) => normalized(item) === "expense")
    .reduce((acc, transaction) => acc + toNumber(transaction.amount), 0);
        ...
        <Row>
            ...
            <div className="card h-100">
              ...
              <div className="card-body">
                {categories.map(category => {
                  const income = transactions.filter(transaction => normalized(transaction) === "credit" && transaction.category === category).reduce((acc, transaction) => acc + toNumber(transaction.amount), 0)
          ...
          <div className="col-lg-3 col-md-6 mb-4">
            <div className="card h-100">
              ...
              <div className="card-body">
                {categories.map(category => {
                  const expenses = transactions.filter(transaction => normalized(transaction) === "expense" && transaction.category === category).reduce((acc, transaction) => acc + toNumber(transaction.amount), 0)
